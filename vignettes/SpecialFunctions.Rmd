...
\begin{figure}
\begin{center}
```{r f7, fig=TRUE,width=5, height=2}
...
par(oldpar)
```
\caption{Dendrogram, production by consensus{#fig:f7}}
\end{center}
\end{figure}
...
\begin{figure}
\begin{center}
```{r f8, fig=TRUE,width=5, height=2}
...
par(oldpar)
```
\caption{Dendrogram, production by hcut(){#fig:f8}}
\end{center}
\end{figure}
...
```{r f9, fig=TRUE, width=4, height=1.5}
...
par(oldpar)
```
\caption{Distribution of the simulated and the original data{#fig:f9}}
...
```{r f10, fig=TRUE, width=4, height=2}
...
\caption{Adjustment curve for the optimal size of plot{#fig:f10}}
...
```{r f11, echo=FALSE, fig=TRUE, width=8, height=2}
...
\caption{Area under the curve (AUDPC) and Area under the Stairs (AUDPS){#fig:f11}}
...
```{r f12, fig=TRUE, width=4.5, height=2.5}
oldpar<-par(mar=c(3,3,4,1),cex=0.7)
#--------------------------
...
\caption{lateblight: LATESEASON{#fig:f12}}
...
In **agricolae**  there are two methods for the study of stability and the AMMI model. These are: a parametric model for a simultaneous selection in yield and stability "SHUKLA'S STABILITY VARIANCE AND KANG'S",  \citet{Kang:1993} and a non-parametric method of Haynes, based on the data range.
