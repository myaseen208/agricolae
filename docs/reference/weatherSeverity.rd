...
<title>Weather and Severity — weatherSeverity • agricolae</title>
...
<meta property="og:title" content="Weather and Severity — weatherSeverity" />
<meta property="og:description" content="Weather and Severity" />
...
<div class="row">
  <div class="col-md-9 contents">
    ...
    <h1>Weather and Severity</h1>
    
    <div class="hidden name"><code>weatherSeverity.rd</code></div>
    </div>
...
<span class='no'>RHthreshold</span>)</pre>
    ...
    <p></p>
<dt>Wfile</dt><dd><p>"Date","Rainfall","Tmp","HumidHrs","humidtmp"</p></dd>
  <dt>Sfile</dt><dd><p>"Cultivar","ApplSys","dates","nday","MeanSeverity","StDevSeverity"</p></dd>
  <dt>EmergDate</dt><dd><p>date</p></dd>
  <dt>EndEpidDate</dt><dd><p>date</p></dd>

    <h2 class="hasAnchor" id="note"><a class="anchor" href="#note"></a>Note</h2>

    <p>All format data for date is yyyy-mm,dd, for example "2000-04-22".
...
<span class='no'>weather</span> <span class='kw'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/r/utils/read.table.html'>read.csv</a></span>(<span class='no'>f</span>,<span class='kw'>header</span><span class='kw'>=</span><span class='fl'>FALSE</span>)</div><div class='output co'>#&gt; <span class='warning'>Warning: file("") only supports open = "w+" and open = "w+b": using the former</span></div><div class='output co'>#&gt; <span class='error'>Error in read.table(file = file, header = header, sep = sep, quote = quote,     dec = dec, fill = fill, comment.char = comment.char, ...): no lines available in input</span></div><div class='input'><span class='no'>f</span> <span class='kw'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/r/base/system.file.html'>system.file</a></span>(<span class='st'>"external/severity.csv"</span>, <span class='kw'>package</span><span class='kw'>=</span><span class='st'>"agricolae"</span>)
<span class='no'>severity</span> <span class='kw'>&lt;-</span> <span class='fu'><a href='https://rdrr.io/r/utils/read.table.html'>read.csv</a></span>(<span class='no'>f</span>)</div><div class='output co'>#&gt; <span class='warning'>Warning: file("") only supports open = "w+" and open = "w+b": using the former</span></div><div class='output co'>#&gt; <span class='error'>Error in read.table(file = file, header = header, sep = sep, quote = quote,     dec = dec, fill = fill, comment.char = comment.char, ...): no lines available in input</span></div><div class='input'><span class='no'>weather</span>[,<span class='fl'>1</span>]<span class='kw'>&lt;-</span><span class='fu'><a href='https://rdrr.io/r/base/as.Date.html'>as.Date</a></span>(<span class='no'>weather</span>[,<span class='fl'>1</span>],<span class='kw'>format</span> <span class='kw'>=</span> <span class='st'>"%m/%d/%Y"</span>)</div><div class='output co'>#&gt; <span class='error'>Error in as.Date(weather[, 1], format = "%m/%d/%Y"): object 'weather' not found</span></div><div class='input'><span class='co'># Parameters dates and threshold</span>
