...
#' 
#' This test is adapted from the Newman-Keuls method. Duncan's test does not
#' control family wise error rate at the specified alpha level. It has more
#' power than the other post tests, but only because it doesn't control the
#' error rate properly. The Experimentwise Error Rate at: 1-(1-alpha)^(a-1);
#' where "a" is the number of means and is the Per-Comparison Error Rate.
#' Duncan's procedure is only very slightly more conservative than LSD. The
#' level by alpha default is 0.05.
#' 
#' It is necessary first makes a analysis of variance.\cr
#' 
#' if y = model, then to apply the instruction:\cr duncan.test(model, "trt",
...
#' parameters} \item{duncan}{Critical Range Table} \item{means}{Statistical
...
#' @references 1. Principles and procedures of statistics a biometrical
#' approach Steel & Torry & Dickey. Third Edition 1997\cr 2. Multiple
#' comparisons theory and methods. Departament of statistics the Ohio State
#' University. USA, 1996. Jason C. Hsu. Chapman Hall/CRC.\cr
#' @keywords htest
#' @importFrom stats quantile qtukey ptukey uniroot
#' @export
#' @examples
#' 
#' library(agricolae)
